...
import { PUBLIC_CONTACT_EMAIL } from '../src/data/contact';
...
<body>
  ...
  <footer class="seo-footer">
    ...
    <p><a href="mailto:${PUBLIC_CONTACT_EMAIL}?subject=Worship%20Word%20Video%20enquiry">Contact: ${PUBLIC_CONTACT_EMAIL}</a> · <a href="mailto:${PUBLIC_CONTACT_EMAIL}?subject=Worship%20Word%20Video%20content%20report">Report a content concern</a></p>
  </footer>
</body>
</html>`;
}
...
const GUIDE_FAQS: Record<string, FaqEntry[]> = {
  '/guides/worship-word-lyrics/': [
    {
      ...
      answer: 'Worship Word Video is free to use and needs no account. Anyone can search and preview videos, build and save a named running order on their device, set tidy start and stop points, and open the separate church-screen window. There is no subscription or fee, and the optional charity donation link is never a condition of use. Videos play in YouTube’s own embedded player, so any adverts or restrictions come from YouTube and the uploader, not from this directory.',
    },
    {
  ...
  '/guides/worship-videos-for-churches-without-musicians/': [
    ...
    },
    {
      ...
      answer: 'Usually fewer than a live band would play; three or four songs is a manageable pattern, because a recording runs to a fixed length and cannot read the room or extend a chorus. In Worship Word Video, anyone can add optional start and stop points to shorten a long opening or a trailing end screen, although YouTube seeks to the nearest keyframe, so treat that timing as approximate. Leave a little silence between songs for prayer.',
    },
    {
      question: 'Can one volunteer run the whole thing on their own?',
      answer: 'Usually yes, because there is no band to rehearse or cue. One volunteer can assemble the running order in advance, then step to the next item when the moment comes. In Worship Word Video the dashboard and its Next control stay on the laptop, and the projection window it opens carries nothing but the player; saved services stay in that browser and need no account. Teach a second volunteer the same routine so no service depends on one person attending.',
    },
  ],
  ...
  '/guides/multilingual-worship/': [
    ...
    },
    {
      ...
      answer: 'Plan a second option for every multilingual item and check each link again shortly before the service. Videos belong to their YouTube uploaders, who can edit, restrict, make private, geo-block or delete them at any time, and embedding can be switched off, so no external link can be promised for a future date. Keep the alternative in the same saved service plan, then preview the whole running order on the church equipment.',
    },
  ],
